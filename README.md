...
- Everything else - YouTube and custom RTMP destinations - is added via the
  **+ Add output** button and shows up in the same list below "Your
  ...
  - **Public Link**: adds a direct, shareable watch link for this channel
    ...
    else's embedded content - only opening it directly works. The random
    token in the URL means the channel id alone (which is also visible in
    the embed code) isn't enough to reach it - there's no "Regenerate"
    button, but deleting the output and adding a new one issues a fresh,
    unguessable link and immediately invalidates the old one.
